...
class RecordingMetadataForm:
    ...
    def get_metadata(self) -> RecordingMetadata | None:
        ...
        self.file_path_entry = ttk.Entry(
            main_frame, textvariable=self.get_recording_audio_file_path_var
        )
        self.file_path_entry.grid(row=0, column=1, sticky="ew", pady=5, padx=(5, 5))
        self.browse_button = ttk.Button(
            main_frame, text="Browse", command=self.__browse_file
        )
        self.browse_button.grid(row=0, column=2, pady=5)

        # Passage / Title
        ttk.Label(main_frame, text="Passage / Title:").grid(
            row=1, column=0, sticky="w", pady=5
        )
        self.get_recording_title_var = tk.StringVar()
        self.title_entry = ttk.Entry(
            main_frame, textvariable=self.get_recording_title_var
        )
        self.title_entry.grid(
            row=1, column=1, columnspan=2, sticky="ew", pady=5, padx=(5, 0)
        )
        ...
        ttk.Label(main_frame, text="Recording Date:").grid(
            row=2, column=0, sticky="w", pady=5
        )
        self.get_recording_date_var = tk.StringVar(
            value=most_recent_sunday.strftime("%Y.%m.%d")
        )
        self.date_entry = ttk.Entry(
            main_frame, textvariable=self.get_recording_date_var
        )
        self.date_entry.grid(
            row=2, column=1, columnspan=2, sticky="ew", pady=5, padx=(5, 0)
        )
        ...
        self.speaker_entry = ttk.Entry(
            main_frame, textvariable=self.get_recording_speaker_var
        )
        self.speaker_entry.grid(
            row=3, column=1, columnspan=2, sticky="ew", pady=5, padx=(5, 0)
        )

        # Submit Button
        self.submit_button = ttk.Button(
            main_frame, text="Submit", command=self.__on_submit
        )
        self.submit_button.grid(row=4, column=0, columnspan=3, pady=20)

        # Validation Messages
        self.validation_label = tk.Label(
            main_frame, text="", fg="red", anchor="w", justify="left"
        )
